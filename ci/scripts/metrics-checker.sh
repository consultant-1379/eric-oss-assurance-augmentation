...
checkHelperTPL(){
    ...
    echo -e "{{- define \"$SERVICE_NAME.prometheus-vars\" -}}" >> .bob/var.compareToLine;
    echo -e "prometheus.io/path: {{ .Values.prometheus.path | quote }}" >> .bob/var.compareToLine;
    echo -e "prometheus.io/port: {{ include \"eric-oss-assurance-augmentation.service-port\" . | quote }}" >> .bob/var.compareToLine;
